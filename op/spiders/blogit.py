...
import json
pattern = r'(\xa0)?'
base = 'https://www.op-media.fi/contentapi/theme-pages?themeId=12952&take=20&skip={}'


class BlogitSpider(scrapy.Spider):
    name = 'blogit'
    offset = 0
    start_urls = [base.format(offset)]
    ITEM_PIPELINES = {
        'blogit.pipelines.OpPipeline': 300,
    ...
    def parse_post(self, response):
        date = response.xpath(
            '(//span[@class="font-default leading-md text-color-default xl:text-color-large text-default xl:text-md"])[last()]/text() | //div[@class="person-block__published"]/span[last()]/text() | //div[@class="max-w-main-content mx-auto"]//text()').get()
